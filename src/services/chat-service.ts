...
const ChatService = {
   ...
   async getConversations(token: string) {
      const { data, error } = await fetchHelper<{ success: boolean; data: Conversation[] }>('/chat/conversations', {
         headers: {
   ...
   async getConversation(conversationId: string, token: string) {
      const { data, error } = await fetchHelper<{ success: boolean; data: Conversation }>(
   ...
   async getConversationMessages(conversationId: string, token: string, page: number = 1, limit: number = 50) {
      ...
      const { data, error } = await fetchHelper<{ data: PaginateResponse<Message> }>(
   ...
   async getUnreadCount(token: string) {
      const { data, error } = await fetchHelper<{ success: boolean; data: { count: number } }>(
   ...
   async searchMessages(query: string, token: string, page: number = 1, limit: number = 20) {
   ...
   async sendMessage(body: SendMessageRequest, token: string) {
      const { data, error } = await fetchHelper<{ success: boolean; data: Message }>('/chat/messages', {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
   ...
   async deleteMessage(messageId: string, token: string) {
      const { data, error } = await fetchHelper<{ success: boolean }>(`/chat/messages/${messageId}`, {
         method: 'DELETE',
         headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
   ...
   // Real-time features
   async sendTypingIndicator(conversationId: string, body: TypingIndicatorRequest, token: string) {
      const { data, error } = await fetchHelper<{ success: boolean }>(`/chat/conversations/${conversationId}/typing`, {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
   ...
   async getActiveUsers(conversationId: string, token: string) {
      const { data, error } = await fetchHelper<{ success: boolean; data: UserModel[] }>(
   ...
   async getUserStatus(userId: string, token: string) {
      const { data, error } = await fetchHelper<{ success: boolean; data: { isOnline: boolean } }>(
